...
function destroy(stream) {
  ...
  if (!(stream instanceof require$$1__default$1['default'])) {
    return stream
  }
...
function SendStream (req, path, options) {
  require$$1__default$1['default'].call(this);
...
util__default['default'].inherits(SendStream, require$$1__default$1['default']);
...
SendStream.prototype.error = function error (status, err) {
  ...
  if (hasListeners(this, 'error')) {
    return this.emit('error', httpErrors$1(status, err, {
      expose: false
    }))
  }
...
var isIP = require$$2__default['default'].isIP;
...
async function update(term) {
    ...
    let currentTag = null;
    try { currentTag = await lib$2.readFile(tagPath, 'utf8'); } catch (e) { }

    let result = null;

    if (!currentTag || !await lib$2.exists(csvPath)) {
    ...
    if (result) {
        ...
        const dst = lib$2.createWriteStream(csvPath, 'utf8');
        await pipe(result.data, dst);
        await lib$2.writeFile(tagPath, result.tag, 'utf8');
    }
...
async function findCourses(term, subjectId, courseId) {
    ...
    let raw = lib$2.createReadStream(filePath, 'utf8');
    let src = readline__default['default'].createInterface({ input: raw });
